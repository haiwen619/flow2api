...
def create_accountpool_router(service: AccountPoolService) -> APIRouter:
    ...
    async def add_account(
        ...
        try:
            item = await service.add_account(
                ...
                twofa_password=request.twofa_password,
                uid=request.uid,
    ...
    async def validate_single_account(
        ...
        try:
            logger.info(
                "[AccountPool][API] single validate request account_key=%s timeout_sec=%s manual=%s headless=%s external_browser=%s bitbrowser=%s",
                account_key,
                timeout_sec,
                manual,
                headless,
            ...
            logger.info("[AccountPool][API] single validate accepted account_key=%s job_id=%s", account_key, job_id)
