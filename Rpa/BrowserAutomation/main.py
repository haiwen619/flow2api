...
TEST_DEFAULT_BITBROWSER_ID = os.environ.get("RPA_TEST_BITBROWSER_ID", "f1ef6f8ccb6146988c67a63b92a78971")
...
class ValidateOptions:
	...
	timeout_sec: int = 300
	slow_mo_ms: int = 0
	user_agent: Optional[str] =  "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/132.0.0.0 Safari/537.36"
...
async def _human_type_text(
	...
	for ch in text:
		...
		key_delay = random.uniform(float(min_key_delay_ms), float(max_key_delay_ms)) / 1000.0
...
def _build_context_kwargs(options: ValidateOptions) -> dict:
	...
	if options.viewport_width and options.viewport_height:
		kwargs["viewport"] = {"width": int(options.viewport_width), "height": int(options.viewport_height)}
	return kwargs
...
async def validate_antigravity_account(
	*,
	...
	entry_url = (os.getenv("ACCOUNTPOOL_RPA_TARGET_URL", "https://labs.google/fx/tools/flow") or "https://labs.google/fx/tools/flow").strip()
	...
	if options.external_browser:
		if options.headless:
			raise RuntimeError("external_browser 模式不支持 headless，请关闭 --headless")
		...
		try:
			...
			if not bitbrowser_id:
				...
				if test_default_id:
					...
					_print_and_log(f"[RPA] 将使用 TEST_DEFAULT_BITBROWSER_ID 复用窗口，ID: {bitbrowser_id}")
			use_bitbrowser = bool(getattr(options, "bitbrowser", True) or bitbrowser_id)
			if not use_bitbrowser:
				raise RuntimeError("当前版本已移除 Playwright 自行启动浏览器；请使用 BitBrowser（options.bitbrowser=True 或指定 bitbrowser_id）")
			if options.headless:
			...
			_print_and_log(f"[RPA] 已通过 CDP 连接到 BitBrowser 窗口，准备打开目标页面...")
			...
			try:
				...
				_print_and_log_exception(f"[RPA] 步骤1/2: 自动登录发生异常: {type(e).__name__}: {e}")
				raise

			# 2) 等待进入目标页面（labs.google/fx）
			_print_and_log(f"[RPA] 步骤2/2: 开始等待进入目标页面，超时 {options.timeout_sec} 秒")
			...
			try:
				...
				if options.manual:
					_print_and_log("[RPA] 步骤2/2: 等待目标页面超时，进入人工介入等待...", level="warning")
					await _wait_until_target_url(page, target_re, timeout_ms=timeout_ms)
					final_url = page.url or ""
					_print_and_log(f"[RPA] 步骤2/2: 人工介入后已进入目标页面 URL: {final_url}")
				else:
					_print_and_log("[RPA] 步骤2/2: 等待目标页面超时且未允许人工介入，抛出超时错误", level="error")
					raise
			...
			try:
				all_cookies = await _collect_google_labs_cookies(context, entry_url=entry_url)
				try:
					cookie_brief = [
						{"name": str(c.get("name") or ""), "domain": str(c.get("domain") or "")}
						for c in all_cookies
					]
				...
				if session_token:
					...
					_print_and_log(f"[RPA] __Secure-next-auth.session-token 内容: {session_token}")
				else:
					_print_and_log(
				...
				_print_and_log_exception(f"[RPA] 提取 session-token 失败: {type(e).__name__}: {e}")
				session_token = None
			...
			if bitbrowser_id:
				try:
					...
					_print_and_log(f"[RPA] 已通过 BitBrowser 关闭窗口，ID: {bitbrowser_id}")
					# 可选：自动删除窗口
					if bool(getattr(options, "bitbrowser_auto_delete", False)):
						deleteBrowser(bitbrowser_id)
						_print_and_log(f"[RPA] 已通过 BitBrowser 删除窗口，ID: {bitbrowser_id}")
				except Exception:
					try:
						log.exception(f"[RPA] BitBrowser 关闭/删除窗口失败，ID: {bitbrowser_id}")
					except Exception:
						pass
	...
	return {
		"success": True,
		"message": ("rpa_login_success" if session_token else "rpa_login_success_but_no_session_token"),
		"error": None,
		"file_path": None,
...
async def _wait_until_target_url(page, target_re: re.Pattern, *, timeout_ms: int) -> None:
...
async def _open_flow_and_click_create(
	*,
	...
	if not clicked:
		_print_and_log("[RPA] 未找到 Create with Flow 按钮，继续后续登录流程", level="warning")
		return page
...
async def _wait_for_flow_entry_ready(page, *, timeout_ms: int = 15000) -> None:
	...
	_print_and_log("[RPA] Flow 入口页加载较慢，未确认按钮可见，继续尝试点击", level="warning")
...
async def _raise_if_google_challenge(page, *, stage: str = "", allow_2fa: bool = False) -> None:
	# 当前项目默认不启用“挑战页即失败”判定，避免误杀正常流程。
	# 如需恢复旧行为，可设置环境变量 ACCOUNTPOOL_RPA_ENABLE_CHALLENGE_BLOCK=1。
	enable_block = str(os.getenv("ACCOUNTPOOL_RPA_ENABLE_CHALLENGE_BLOCK", "0")).strip().lower() in (
		"1",
		"true",
		"yes",
		"on",
	...
	raise AccountInvalidError(f"出现了意外的验证码|手机号验证，账号可能已作废：{reason}{stage_msg}")
	...
	*,
	...
	try:
		...
		if await page.locator(pwd_sel).count() > 0:
			...
			clicked = await _click_first_visible(
				page,
				["#passwordNext", "button:has-text('下一步')", "button:has-text('Next')", "div[role='button']:has-text('Next')"],
				timeout_ms=6000,
			)
			if not clicked:
				try:
					...
					pass
			await _raise_if_google_challenge(page, stage="after_click_password_next", allow_2fa=True)
			if is_2fa_enabled:
	...
	await _raise_if_google_challenge(page, stage="login_end", allow_2fa=bool(is_2fa_enabled))
...
async def _is_google_2fa_page(page) -> bool:
	...
	try:
		url = (page.url or "").lower()
		if any(x in url for x in ["/challenge/selection", "/challenge/totp", "/challenge/ipp", "/challenge/ootp"]):
			return True
	except Exception:
		pass
	try:
...
async def _wait_for_2fa_code_input_selector(page, *, timeout_ms: int = 12000) -> Optional[str]:
	...
	return await _find_2fa_code_input_selector(page, selectors=selectors, timeout_ms=timeout_ms)


async def _find_2fa_code_input_selector(page, *, selectors: list[str], timeout_ms: int = 12000) -> Optional[str]:
...
def _fetch_2fa_code_from_cn(twofa_password: str, *, timeout_sec: int = 15) -> Optional[str]:
	"""调用 2fa.cn 接口获取动态验证码。"""
...
async def _try_google_2fa_flow(page, *, twofa_password: str, options: ValidateOptions) -> None:
	"""若出现 2FA 页面，自动调用 2fa.cn 拉取 code 并填入继续。"""
	secret = (str(twofa_password or "").strip() or "")
	if not secret:
		_print_and_log("[RPA] 2FA 已启用但未提供 twofa_password，跳过自动填码", level="warning")
		return
	...
	if not await _try_select_google_authenticator_option(page, timeout_ms=8500):
		_print_and_log("[RPA] 未能可靠点击 Google Authenticator 选项，继续尝试检测验证码输入框", level="warning")
	...
	for attempt in range(2):
		...
		try:
			code = await asyncio.to_thread(_fetch_2fa_code_from_cn, secret, timeout_sec=15)
		except Exception as e:
			code = None
			last_error = str(e)
...
async def _try_select_google_authenticator_option(page, *, timeout_ms: int = 12000) -> bool:
	...
	if await _find_2fa_code_input_selector(page, selectors=quick_code_selectors, timeout_ms=180):
		return True
	...
	try:
		...
		if fast_clicked and await _wait_for_2fa_code_input_selector(page, timeout_ms=2200):
			return True
	except Exception:
		pass
...
async def _wait_for_google_identifier_step(page, *, timeout_ms: int = 12000) -> Optional[str]:
...
async def _set_input_value_robust(page, *, selectors: list[str], text: str, timeout_ms: int = 10000) -> bool:
...
async def _best_effort_google_consent(page, *, options: ValidateOptions) -> None:
	...
	try:
		...
		for idx in range(min(cnt, 12)):
			b = btns.nth(idx)
			try:
				label = await b.locator("span[jsname='V67aGc']").first.inner_text(timeout=180)
			except Exception:
				try:
...
def _build_arg_parser() -> argparse.ArgumentParser:
	p = argparse.ArgumentParser(description="账号池 RPA 验活（Antigravity OAuth + 凭证落盘）")
	p.add_argument("--username", default="", help="Google 登录账号（通常是邮箱）")
	p.add_argument("--password", default="", help="Google 登录密码")
	p.add_argument("--user-session", default="rpa-cli", help="用于隔离 OAuth 流程的会话标识")
	p.add_argument("--headless", action="store_true", help="无头模式")
	p.add_argument("--manual", action="store_true", help="允许人工介入（遇到验证码/2FA 时可用）")
	p.add_argument("--timeout", type=int, default=300, help="超时秒数（等待进入目标页面）")
	...
	p.add_argument("--human-delay-min", type=float, default=0.0, help="随机延迟最小秒数")
	p.add_argument("--human-delay-max", type=float, default=0.0, help="随机延迟最大秒数")
	p.add_argument(
		"--external-browser",
		action="store_true",
...
async def _amain(argv: list[str]) -> int:
	...
	arg_bit_id = (str(args.bitbrowser_id).strip() if args.bitbrowser_id is not None else "")
