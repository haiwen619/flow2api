...
- Python 3.11（推荐）
- 可联网环境（调用上游接口）
- 默认 `config/setting_example.toml` 中 `captcha_method = "browser"`，因此本地部署通常还需要安装 Playwright 浏览器内核
- 当前不要使用 `Python 3.14` 安装依赖；已锁定的 `pydantic-core`、`curl-cffi` 等依赖会在该版本上回退到源码构建，容易直接失败
...
# 2) Linux/macOS 推荐先创建 Python 3.11 虚拟环境
...
- 同一自动刷新窗口内仅触发一次自动刷新，避免后台每分钟重复执行
...
- `Failed building wheel for pydantic-core`：通常是因为安装环境用了 `Python 3.14`。请切换到 `Python 3.11` 虚拟环境后重新执行 `pip install -r requirements.txt`
